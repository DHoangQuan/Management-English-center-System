...
(function () {
	...
	ScheduleTemplate.prototype.initEvents = function() {
	    var self = this;
	    var x = document.querySelectorAll(".schedule");
	    for (var i = 0; i < x.length; i++) {
	        x[i].addEventListener("click", function () {
	            time = this.getAttribute("data-tim");
	...
	ScheduleTemplate.prototype.loadEventContent = function(content) {
		...
		if (slot == null) {
		    httpRequest.open('GET', '/UsingRooms/IndexLesson?lessonid=' + lessonid + '');

		} else {
		    console.log(slot);
		    httpRequest.open('GET', '/UsingRooms/IndexGet?slot=' + slot + '&date=' + time + '');
		}
    httpRequest.send();
	};
